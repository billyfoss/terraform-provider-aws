...
* `instance_interruption_behavior` - (Optional) Indicates whether a Spot instance stops, hibernates or terminates when it is interrupted. Default is `terminate` as this is the current AWS behaviour.
